 ...
 * @link    www.gxcms.com
 */
class ColRuninfoModel extends AdvModel {
    private $DB;
    private $ContDB;
    private $UrlDB;
    private $CModel;
    function __construct(){
		...
		$this->CModel =D('Admin.CustomCollectinfo');
		//Load('extend');
    }
    ...
    function ColVideoList($where){
		...
		$where[C('db_prefix').'co_content.class']= array('eq',1); //资讯采集分类
		...
		$video_url   = U('Admin-CustomCollectinfo/ColVideo',array('nid'=>urlencode($where['nid']),'title'=>urlencode($title),'p'=>''),false,false);
    ...
    function CoUrl(&$Data){ 
       ...
       $TotalPage = count($ListUrls); 
			if ($TotalPage > 0) {
				...
				$ArtUrl  = $this->GetArtlist($CurList, $Data);//资讯url	
				...
				foreach ($ArtUrl as $v) {//处理预定义及html标签
					...
					if($Data['action']==='real'){//入库
						...
						if (!$this->UrlDB->where($where)->find()) {//若不存在,更新入库
							...
							$this->ContDB->data(array('nid'=>$where['nid'], 'status'=>0, 'class'=>1, 'url'=>$v['url'], 'title'=>$v['title'],'addtime'=>time()))->add();					
						    $newNum++;
     ...
     function Collecting(&$config){
			...
			if (!empty($list) && is_array($list)) {
			   ...
			   if ($total_page <$page) {
	            ...
	            $StrUrl='?s=Admin/CustomCollectinfo/ColRun/action/real/type/content/nid/'.$config['id'].'/page/'.($page+1).'/total/'.$all.'/stime/'.$config['stime'];
			    ...
			    if ($total_page >=$page){//添加缓存--续采
			    	$CacheUrl='?s=Admin/CustomCollectinfo/ColRun/action/real/type/content/nid/'.$config['id'].'/page/'.$page.'/total/'.$all.'/stime/'.$config['stime'];
