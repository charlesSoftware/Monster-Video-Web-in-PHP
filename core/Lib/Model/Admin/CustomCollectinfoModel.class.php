 ...
 * @link    www.gxcms.com
 */
class CustomCollectinfoModel extends Model {
    private $DB;
    private $CDB;
    private $VdoDB;
    private $ContDB;
	...
	function ShowList(){	
		...
		$node_url   = U('Admin-CustomCollectinfo/ListShow',array('keyword'=>urlencode($keyword),'p'=>''),false,false);
   ...
   //分页显示自定义栏目分类转换
	 ...
	 */
     ...
     function UpCheck($url,&$html,$nid){
     	 if(empty($html['title'])) {$this->error='资讯标题为空!';return false;}
     	 //检测info库中是否已含有该源链接的资讯 	  
	     $ArrUrl=$this->VdoDB->field('id,title,reurl')->where("reurl='".$url."'")->find();    
	     if(!$ArrUrl){//若没有相同源链接资讯,则检测资讯标题
	        $ArrTitle=$this->VdoDB->field('id,title,reurl')->where("title='".$html['title']."'")->find();
	        if($ArrTitle){ 
