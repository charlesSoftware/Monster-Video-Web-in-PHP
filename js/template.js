$(document).ready(function(){
	...
	//内容页主演相关影视
	if($("#actor_video").length>0){
		$("#actor_video").load($("#actor_video").attr('href'));	
	}
	//内容页导演相关影视
	if($("#director_video").length>0){
		$("#director_video").load($("#director_video").attr('href'));	
	}
	//内容页大家都在看影视
	if($("#everyone_video").length>0){
		$("#everyone_video").load($("#everyone_video").attr('href'));	
	}
	//内容页同类热门新闻
...
function boxopen(index){
	$(".operation .openbox").css("display","none");
	if(index==1){$("#sharebox").css("display","block");}
	if(index==2){$("#urlbox").css("display","block");}
	if(index==3){$("#errorbox").css("display","block");}
	$(".operation .off").click(function(){$(".operation .openbox").css("display","none");});
}